<template>
  <section id="prev-next" class="prev-next btn-group grid grid-cols-2">
   <!-- if prev data is available display the link -->
   <nuxt-link
     v-if="prev"
     :to="{ name: 'blog-slug', params: { slug: prev.slug } }"
     class="prev btn btn-outline btn-success"
   >
   ...
   <span class="prev btn btn-outline" v-else></span>

   <!-- if prev data is available display the link -->
   <nuxt-link
     v-if="next"
     :to="{ name: 'blog-slug', params: { slug: next.slug } }"
     class="next btn btn-outline btn-success"
   >
   <span class="icon-cont">
     <svg
     xmlns="http://www.w3.org/2000/svg"
   ...
   <!-- else display empty span for styling purposes -->
   <span class="next btn btn-outline" v-else></span>
 </section>
</template>
...
<style scoped>
   ...
   .prev-next {
     @apply flex gap-12 py-8 items-center justify-between m-auto max-w-xl max-w-4xl;
   }
